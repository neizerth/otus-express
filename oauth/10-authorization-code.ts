/**
 * OAuth 2.0 Authorization Code — один сервер играет провайдера и клиента.
 * Внешние сервисы не нужны: весь поток можно пройти в браузере.
 */
...
// http://localhost:3110/oauth/authorize?client_id=...&redirect_uri=...&response_type=code
...
// curl -X POST http://localhost:3110/oauth/token ...
...
// curl -H "Authorization: Bearer TOKEN" http://localhost:3110/oauth/user
...
// http://localhost:3110/callback?code=...
...
app.listen(PORT, () =>
	console.log(
		`oauth: http://localhost:${PORT}\n  GET / — старт\n  GET /oauth/authorize — провайдер\n  POST /oauth/token — обмен code → token`,
	),
);
